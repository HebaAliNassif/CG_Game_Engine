    ...
    public:
        ...
        glm::mat4 getViewMatrix(){
            if(dirtyFlags & V_DIRTY){ // Only regenerate the view matrix if its flag is dirty
                V = glm::lookAt(camera_transform->getPosition(), camera_transform->getPosition() + camera_transform->getForward(), up);
        ...
        [[nodiscard]] glm::vec3 getEyePosition() const {return camera_transform->getPosition();}
...
#endif //COMPUTER_GRAPHICS_PROJECT_CAMERA_TEST_H
/*
        // Setup the camera as a perspective camera
        void setupPerspective(float field_of_view_y, float aspect_ratio, float near, float far){
            this->type = CameraType::Perspective;
